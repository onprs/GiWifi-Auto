...
面向山东科技大学 GiWiFi 的 OpenWrt 自动认证工具。程序由系统服务持续检查网络状态，在检测到 Portal 时自动完成认证，并支持多账号管理、状态查询、事件日志和终端界面。完整硬件与使用教程见 [OpenWrt 从零使用与完整校园网配置教程](./openwrt/README.md)。

## 功能

- 多账号自动分配到不同 OpenWrt 上联线路进行认证
- 认证失败自动退避重试，认证自动保活
- 多账号独立启用、停用和手动触发
- 近期事件查询和实时日志跟随
...
程序会根据 OpenWrt 当前运行状态自动识别可用上联线路和 IPv4 路由，并按线路优先级为账号分配认证线路。添加账号时只需填写用户名和密码，无需填写 VLAN、网卡或路由信息。

主界面会显示账号与接口的对应关系、接口是否存在、管理状态、物理载波、运行状态、IPv4 地址、认证状态、最近结果、重试次数和下次检查时间。底部会显示当前可用操作；按 `?` 可查看完整操作列表。按 `D` 或 `Delete` 删除当前账号。
